...
namespace Sefate.Incubator.WorkItem.DAL
{
    public class IncubatorWorkitemEntitiesManager
    {
        ...
        public bool AddNewField(int? workItemId, string fieldCode, string fieldValue, string fieldSetCode, string mapCode, string componentRef, bool isRelated, int? quarterID = null,int? instance=null)
        {
            bool result = false;
            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
        ...
        public bool UpdateQuardtrandScoreCard(int score, int workItem, int quardrant)
        {
            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
                var items = from a in currentContext.IncubatorQuarters
                            where a.IncubatorQuardrantID == quardrant && a.WorkItemID == workItem
                            select a;
		...
		public bool UpdateScore(int score,int quardrantID)
		{
			bool result = false;
			using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
			{
        ...
        #region Intervention Report

        public bool AddReportTemplate(string name,double verssion,byte[] content)
        {

            bool result = false;
            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
                var template = new InterventionReportTemplate()
                {
                     Name = name,
                ...
                currentContext.InterventionReportTemplates.Add(template);
                result = currentContext.SaveChanges() > 0;
            }
            return result;
            }

        public List<InterventionReportField> GetReportFields(int reportID)
        {
            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
                var items = (from a in currentContext.InterventionReportFields
                             where a.InterventionReportID == reportID
                             select a);
        ...
        public bool AddNewReportField(string code, int reportID, Guid issueID, string value, string workItemFieldCode,string mapCode)
        {
            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
                InterventionReportField field = new InterventionReportField()
                {
                    FieldCode = code,
                    InterventionReportID = reportID,
                    IssueID = issueID,
                    FieldValue = value,
                    WorkItemFieldCode = workItemFieldCode,
                    MapCode = mapCode

                };
                currentContext.InterventionReportFields.Add(field);
                return currentContext.SaveChanges() > 0;
            }
        }

        public bool UpdateReportField(string FieldValue, int ID)
        {
            bool result = false;

            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
                var fieldList = from a in currentContext.InterventionReportFields
                                where a.ID == ID
                                select a;
                if (fieldList != null && fieldList.Any())
                {
        ...
        public InterventionReportTemplate GetTemplate()
        {
            using (var currentContext = DBConnector.Get<IncubatorClientsEntities>())
            {
                var items = (from a in currentContext.InterventionReportTemplates
                             orderby a.Verssion descending
                             select a);
