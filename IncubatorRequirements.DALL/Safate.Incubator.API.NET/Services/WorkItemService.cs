...
namespace Safate.Incubator.API.Core.Services
{
    public class WorkItemService : IWorkItemService
	{
		...
		public bool UpdateStatus(StageUpdateView item)
		{
            ...
            {
                incubatorWorkitemEntitiesManager = new IncubatorWorkitemEntitiesManager();
                result = incubatorWorkitemEntitiesManager.UpdateWorkItemStage(item.ToStage, item.WorkItem.WorkItemID, item.CurrentUser);
                result &= incubatorWorkitemEntitiesManager.UpdateWorkitemViewStatus(item.WorkItem.WorkItemID);
            }
			return result;
		}
	}
