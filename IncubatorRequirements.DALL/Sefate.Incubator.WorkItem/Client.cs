...
namespace Sefate.Incubator.WorkItem
{
    public class Client
    {
        ...
        public Client(int workItem,bool isWorkItem)
        {
            incubatorWorkitemEntitiesManager = new IncubatorWorkitemEntitiesManager();

            var dbWorkItem = incubatorWorkitemEntitiesManager.GetWorkItemByID(workItem);

            if (dbWorkItem != null && dbWorkItem.ClientID.HasValue)
            {
                var client = incubatorWorkitemEntitiesManager.GetClientByID(dbWorkItem.ClientID.Value);
            }
        }
